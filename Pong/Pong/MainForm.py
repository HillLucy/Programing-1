...
class MainForm(Form):
	...
	def InitializeComponent(self):
		...
		self._lbltitle.SizeChanged += self.LbltitleSizeChanged
		# 
		# leftscore
		# 
	...
	def TimerballTick(self, sender, e):
		...
		if ball.Location.X >= self.Width or (ball.Location.X > rpdl.Right + 20 and ball.Location.Y > rpdl.Top):
			...
			if lscore == 10: # Left win position
	...
	def MainFormKeyDown(self, sender, e):
		...
		""" TODO: FINSIH MULTIPLAYER CONTROLS"""
		if tmult.Enabled and tball.Enabled:
			""" FOR BOTH KEYS, ASLO NEED TO SET tbool.Enabled = False"""
			if e.KeyCode == Keys.S:
				pass
			if e.KeyCode == Keys.W:
				pass
	...
	def MainFormLoad(self, sender, e):
		""" TODO: ADD 3 UNIQUE SECRETS/CHEATS/EASTER EGGS IN TOTAL AND FINISH MULTIPLAYER AND SCOREBOARD IN DUMMY AI """
		self.balld = 1
		self.ballup = self.R.Next(-4, 5)
		self._timerball.Enabled = False
	...
	def TimerleftTick(self, sender, e):
		self.pdlTick(self._lblleft, self._flagleft, self._timerleft)

	def TimerrightTick(self, sender, e):
		self.pdlTick(self._lblright, self._flagright, self._timerright)
	...
	def LbltitleSizeChanged(self, sender, e):
		pass

	def MainFormSizeChanged(self, sender, e):
		self._lblright.Left = self.Width - 25 - self.lblright.Width
		self._lblball.Left = self.Width // 2
		self.lblball.Top = self.Height = self.Height // 2
